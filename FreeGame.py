...
config = "/Users/connorkelly/Documents/Work/PythonWorkspace/FreeGamesMirrorBonuses/config.xlsx"
n_mirror_table = pd.read_excel(config, 'data', usecols=[0,1])
bonus_table = pd.read_excel(config, 'data', nrows = 6, usecols=[3,4])
stored = bonus_table.bweights
bonus_table_inital = pd.read_excel(config, 'data', nrows = 6, usecols=[6,7])
results_file = open("DracsStacksResults"+str(date.today())+str(st)+".txt", "w")

# Globals
mirror_type = ["Coin","Multiplier","PlusSpins","Transform","Collect","SuperScatter"]
accumulated = [0,0,0,0,0,0] # Coins, Multiplier, PlusSpins, Transforms, Collect, SuperScatter
...
def bonus_choice(mirror_count, collect_count, transform_count, new_selects, first_placement, reels):
    ...
    for i in range(new_selects):
        ...
        if "SuperScatter" in list(itertools.chain(*reels)) or superscattervalid == False:
            bonus_table.bweights[5] = 0
        if full_reels >= 4 or transform_count == 1 or mirror_count >= 24:
...
def bonus_assessment():
    ...
    accumulated[5] = bonus_counter(reels)[5]
...
def main(sim_flag,coin_e,multiplier_e,plus_spin_e): 
    ...
    results_file.write(f"{total_bet_return}\t{accumulated[0]}\t{accumulated[1]}\t{plus_spin_value}\t{accumulated[3]}\t{accumulated[4]}\t{accumulated[5]}\t{inital_mirror_choice}\t{final_mirror_count}\n")
